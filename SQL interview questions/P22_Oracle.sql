Write a query that compares each employee's salary to their manager's and the average department 
salary (excluding the managers salary). Display the department, employee ID, employee's salary,
 manager's salary, and department average salary. Order by department, then by employee salary 
 (highest to lowest).
...
select b.department, b.id, b.salary, b.manager_id,
CASE WHEN b.id <> b.manager_id then c.salary else null end as manager_salary, ROUND(avg_salary)
from employee_o b
left join 
employee_o c on c.id = b.manager_id
left join
(select department, AVG(CASE WHEN employee_title <> 'Manager' THEN salary else null end) as avg_salary 
from employee_o
group by 1
) a ON
