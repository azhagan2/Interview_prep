...
WITH CTE AS
(select *, LEAD(timestamp, 1) OVER(PARTITION by cust_id order by timestamp) as logout_time
from customer_state_log a 
order by cust_id, timestamp), 
CTE2 AS
(select *, logout_time - timestamp as active_hours from CTE where state = 1)

 select cust_id, SUM(active_hours)
 from CTE2
 group by 1
