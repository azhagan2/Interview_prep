...
Naïve forecast is one of them. To create a naïve forecast for "distance per dollar" 
(defined as distance_to_travel/monetary_cost), first sum the "distance to travel" and 
"monetary cost" values monthly. This gives the actual value for the current month.
For the forecasted value, use the previous month's value. After obtaining both actual 
and forecasted values, calculate the root mean squared error (RMSE) using the formula 
RMSE = sqrt(mean(square(actual - forecast))). Report the RMSE rounded to two decimal places.

🔍 At first glance, this might seem tedious, but it's straightforward once you break 
it down. Give it a try! 👇
...
WITH CTE AS
(select SUBSTRING(request_date::text, 1,7) as month, SUM(distance_to_travel) tot_distance_to_travel,
SUM(monetary_cost) tot_monetary_cost, 
sum(distance_to_travel) / sum(monetary_cost) dpd
from uber_request_logs
GROUP BY 1),

CTE2 as 
(select *, LAG(dpd, 1) over(order by month) as forcasted_dpd
from CTE)

select ROUND(SQRT(AVG(POWER((dpd::numeric-forcasted_dpd::numeric),2))),2) as "RSME"
from CTE2
where forcasted_dpd is not null