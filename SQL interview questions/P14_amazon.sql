Find all records from,  days when the number of distinct users receiving emails was greater 
than the number of distinct users sending emails.
...
group by 1) b 
on a.day = b.day
where b.distinct_receivers > distinct_senders
order by a.day