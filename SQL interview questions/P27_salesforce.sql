Find the highest salary among the salaries which are unique (salaries which appeared once)
...
select max(salary) highest_unique_salary
from 
(select salary, count(salary) cnt
from employee
group by 1) a 
where cnt = 1