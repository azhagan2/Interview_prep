Find the users who purchased second time within 7 days of their first purchase with us
...
select user_id
from 
(select *, LEAD(created_at) OVER(PARTITION BY user_id ORDER BY created_at) nxt_purchase
from amazon_transactions
order by user_id, created_at)
where (nxt_purchase::date - created_at::date) <= 7