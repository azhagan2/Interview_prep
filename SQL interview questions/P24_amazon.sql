American Express is reviewing their customers transactions, and you have been tasked with 
locating the customer who has the third highest total transaction amount. The output should 
include the customers id, as well as their first name and last name. 
...
from
(select *, DENSE_RANK() OVER(ORDER BY sum DESC) rk 
from 
(select a.id, a.first_name, a.last_name, SUM(total_order_cost)
from customers a 
left join card_orders b on a.id = b.cust_id 
group by 1,2,3) a ) b 
where rk = 3