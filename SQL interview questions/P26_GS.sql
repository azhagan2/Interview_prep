...
WITH CTE AS
(select arrival_time as event_time, 1 as event_type from train_arrivals 
union all
select departure_time as event_time, -1 as event_type from train_departures
)

select max(platforms) min_platforms
from 
(select event_time, SUM(event_type) OVER(order by event_time) as platforms
from CTE )